...
from math_part import translate_origin_to_cannon, translate_image_point, sin, cos, find_angle_with_drag, radians, \
    memory_size, pi
...
def start_aiming(image=None, images_array=None):
    ...
    if coords.size:
        ...
        d_camera, M_coords = distance(real_points, focus=focus_length, qrcode_length=qr_width, image_size=image_resol,
                                      return_middle=True)
...
if __name__ == '__main__':
    ...
    coordinates_of_camera_with_respect_to_the_cannon = (
        0.05, -0.09, 0.019)  # Cannon itself, needed for correct vertical angle
    ...
    # Process that reads and shows images
        ...
        while 1:
            ...
            if control_values['x']:
                if cannon.check_command():
                    ...
                    if current_sound_mode == 'starcraft':
                        play_random_sound_from_selection(sounds_shoot, channel_id=0, wait=True,
                                                         selection=['tta-ta-ta-da', 'justice'])
                    elif current_sound_mode == 'starwars':
                        play_random_sound(starwars_sounds, channel_id=0)
            else:
